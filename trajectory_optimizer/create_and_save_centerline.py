...
#Location of csv to load in
csv_file = 'traj_optimizer_inputs/tracks/custom.csv'
...
with open(csv_file, 'r') as file:
            ...
            for row in csvreader:
                ...
                width_1.append(float(row[2]))
                width_2.append(float(row[3]))
x= np.array(x)
y=np.array(y)
width_1=np.array(width_1)
width_2=np.array(width_2)

#Scale up track to meet real f1 scaling
csv_out = 'inputs/tracks/levine_scaled.csv'
with open(csv_out, 'w', newline='') as csvfile:
    mapwriter = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
    
    mapwriter.writerow(['# x_m','y_m','w_tr_right_m','w_tr_left_m'])
    for i in range(len(x)):
            line = [x[i]*10,y[i]*10, width_1[i]*10, width_2[i]*10]
            mapwriter.writerow(line)
...
file_paths["track_name"] = "levine_scaled"#"berlin_2018"                                    # Berlin Formula E 2018
...
file_paths["track_file"] = os.path.join(file_paths["module"], "traj_optimizer_inputs", "tracks", file_paths["track_name"] + ".csv")

# assemble friction map import paths
file_paths["tpamap"] = os.path.join(file_paths["module"], "traj_optimizer_inputs", "frictionmaps",
                                    file_paths["track_name"] + "_tpamap.csv")

if mintime_opts["tpadata"] is None:
    file_paths["tpadata"] = os.path.join(file_paths["module"], "traj_optimizer_inputs", "frictionmaps",
                                         file_paths["track_name"] + "_tpadata.json")
else:
    file_paths["tpadata"] = os.path.join(file_paths["module"], "traj_optimizer_inputs", "frictionmaps", mintime_opts["tpadata"])
...
if not (opt_type == 'mintime' and not mintime_opts["recalc_vel_profile_by_tph"]):
    file_paths["ggv_file"] = os.path.join(file_paths["module"], "traj_optimizer_inputs", "veh_dyn_info", pars["ggv_file"])
    file_paths["ax_max_machines_file"] = os.path.join(file_paths["module"], "traj_optimizer_inputs", "veh_dyn_info",
...
with open(csv_file, 'r') as file:
            ...
            for row in csvreader:
                rows.append(row)
goal_points=np.array(rows)

x = goal_points[:,0].astype(float)
y = goal_points[:,1].astype(float)
diff_array = abs(goal_points[:,2].astype(float)+goal_points[:,3].astype(float))
...
x_race=[]
y_race=[]
with open("outputs/traj_race_cl.csv", 'r') as file:
            ...
            for row in csvreader:
                count+=1
                if count >2:
                    x_race.append(float(row[1])/10)
                    y_race.append(float(row[2])/10)
                


#Rescale CSV
with open('outputs/final_raceline_output.csv', 'w', newline='') as csvfile:
    mapwriter = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
    
    
    for i in range(len(x_race)):
        line = [x_race[i]/10, y_race[i]/10]
        mapwriter.writerow(line)
...
plt.figure(100)
plt.clf()
plt.scatter(x,y,linewidth= 45, c='gray')
plt.plot(x_race, y_race,c='green', linewidth=2)
